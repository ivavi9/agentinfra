...
write-secret: config-check
	...
	KUBECONFIG=$(KUBECONFIG_PATH) $(KUBECTL) create secret generic gemini-ai-proxy-config \
		--from-literal=config="{\"route_type\": \"llm/v1/chat\", \"auth\": {\"allow_override\": false, \"param_name\": \"key\", \"param_value\": \"$$key\", \"param_location\": \"query\"}, \"model\": {\"provider\": \"gemini\", \"name\": \"gemini-flash-latest\"}}" \
